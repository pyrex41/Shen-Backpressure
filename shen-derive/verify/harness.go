...
type HarnessConfig struct {
	...
	// Seed enables seeded random sampling on top of the deterministic
	// boundary pool. Zero (the default) means "deterministic, boundary
	// values only" — identical behavior to pre-seeding shen-derive. Any
	// non-zero value is used as the RNG seed and triggers RandomDraws
	// additional primitive samples per type.
	Seed int64

	// RandomDraws is the number of random primitive samples to layer on
...
func BuildHarness(cfg *HarnessConfig) (*Harness, error) {
	...
	// Build a sampling context. A zero Seed keeps the old behavior:
	// deterministic boundary values only, no random draws.
	sctx := &SampleCtx{TT: cfg.TypeTable}
	if cfg.Seed != 0 {
		sctx.Rand = rand.New(rand.NewSource(cfg.Seed))
		sctx.RandomDraws = cfg.RandomDraws
		if sctx.RandomDraws <= 0 {
			sctx.RandomDraws = 8
		}
	}
