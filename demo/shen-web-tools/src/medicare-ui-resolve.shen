...
(define panel-props
  ...
  "followup"    Data _ -> [["suggestions" (generate-followups Data)]]
  _             _    _ -> [])
...
(define generate-followups
  \doc "Generate suggested follow-up questions based on current data."
  Data ->
    (let PlanType (get-prop "planType" Data)
         Zip (get-prop "zip" Data)
         Filter (get-prop "filter" Data)
      (make-followup-string PlanType Zip Filter)))

(define make-followup-string
  \doc "Build pipe-separated follow-up suggestions."
  "advantage" Zip _ ->
    (cn "What's the out-of-pocket maximum?|"
    (cn "Do these plans include dental and vision?|"
    (cn "Compare with Original Medicare|"
        "Which plan has the lowest monthly premium?")))

  "part-d" Zip _ ->
    (cn "What drugs are in the formulary?|"
    (cn "What's the donut hole coverage gap?|"
    (cn "Compare Part D plans by premium|"
        "Does this cover insulin?")))

  "supplement" Zip _ ->
    (cn "What's the difference between Plan F and Plan G?|"
    (cn "Which Medigap plan has the lowest premium?|"
    (cn "What does Medigap NOT cover?|"
        "Compare with Medicare Advantage")))

  _ Zip _ ->
    (cn "What are the 2025 premiums?|"
    (cn "Compare plan types|"
    (cn "What's covered under this plan?|"
