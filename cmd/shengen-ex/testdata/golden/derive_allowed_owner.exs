# Code generated by shengen-ex derive from cmd/shengen-ex/testdata/features.shen (define allowed-owner?). DO NOT EDIT.
#
# Spec-equivalence test: Feat.Impl.allowed_owner?/2 must agree with the Shen
# definition of allowed-owner? evaluated in-process by shen-erl (the Shen kernel
...
# sibling shen-erl checkout. Signature: {account-id --> (list workflow-step) --> boolean}
defmodule Feat.Impl.AllowedOwnerSpecTest do
  ...
  @spec_path "cmd/shengen-ex/testdata/features.shen"
  @shen_fun :"allowed-owner?"
  @max_cases 500
  ...
  test "allowed-owner? agrees with Feat.Impl.allowed_owner?/2 on the boundary pool", %{oracle: oracle} do
    cases = cases()
    ...
    failures =
      ...
      |> Enum.flat_map(fn {[a1, a2] = args, index} ->
        ...
        impl = Feat.Shen.Term.to_shen(Feat.Impl.allowed_owner?(a1, a2))
  ...
  property "allowed-owner? agrees with Feat.Impl.allowed_owner?/2 on StreamData draws", %{oracle: oracle} do
    check all a1 <- gen_account_id(), a2 <- gen_list_of_workflow_step(), max_runs: 100 do
      assert oracle_call(oracle, [a1, a2]) == Feat.Shen.Term.to_shen(Feat.Impl.allowed_owner?(a1, a2))
    end
  end

  defp cases do
    product = cartesian([pool_account_id(), pool_list_of_workflow_step()])
  ...
  defp pool_string do
    ["", "a", "b", "tenant-1", "tenant-2", "Tenant-1", " ", "ünïcødé"]
  end

  defp pool_account_id do
    for f1 <- pool_string(),
        {:ok, value} <- [Feat.Shen.AccountId.new(f1)],
        do: value
  end

  defp pool_number do
    [0, 1, -1, 2, 0.5, -0.5, 100, 1.0e9, 10, 3]
  end

  defp pool_amount do
    for f1 <- pool_number(),
        {:ok, value} <- [Feat.Shen.Amount.new(f1)],
        do: value
  end

  defp pool_step_wait do
    for f1 <- pool_amount(),
        {:ok, value} <- [Feat.Shen.StepWait.new(f1)],
        do: value
  end

  defp pool_step_call do
    for f1 <- sample(pool_account_id(), 4),
        f2 <- sample(pool_number(), 4),
        {:ok, value} <- [Feat.Shen.StepCall.new(f1, f2)],
        do: value
  end

  defp pool_workflow_step do
    sample(pool_step_wait(), 8) ++
      sample(pool_step_call(), 8)
  end

  defp pool_list_of_workflow_step do
    xs = pool_workflow_step()
    picks = sample(xs, 8)
  ...
  defp gen_account_id do
    StreamData.one_of([StreamData.member_of(pool_account_id()), StreamData.map(gen_string(), &Feat.Shen.AccountId.new/1)])
    |> StreamData.map(fn
      {:ok, value} -> value
      {:error, _} -> nil
      value -> value
  ...
  defp gen_number do
    StreamData.one_of([StreamData.member_of(pool_number()), StreamData.integer(), StreamData.float()])
  end

  defp gen_amount do
    StreamData.one_of([StreamData.member_of(pool_amount()), StreamData.map(gen_number(), &Feat.Shen.Amount.new/1)])
    |> StreamData.map(fn
      {:ok, value} -> value
      {:error, _} -> nil
      value -> value
  ...
  defp gen_step_wait do
    StreamData.member_of(pool_step_wait())
  end

  defp gen_step_call do
    StreamData.member_of(pool_step_call())
  end

  defp gen_workflow_step do
    StreamData.member_of(pool_workflow_step())
  end

  defp gen_list_of_workflow_step do
    StreamData.list_of(gen_workflow_step(), max_length: 5)
  end
