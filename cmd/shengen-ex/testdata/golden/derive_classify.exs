# Code generated by shengen-ex derive from cmd/shengen-ex/testdata/features.shen (define classify). DO NOT EDIT.
#
# Spec-equivalence test: Feat.Impl.classify/1 must agree with the Shen
# definition of classify evaluated in-process by shen-erl (the Shen kernel
# on the BEAM) on a deterministic boundary pool.
#
# Oracle location: $SHEN_ERL_EBIN, else $SHEN_ERL_ROOT/ebin, else a
# sibling shen-erl checkout. Signature: {number --> symbol}
defmodule Feat.Impl.ClassifySpecTest do
  ...
  test "classify agrees with Feat.Impl.classify/1 on the boundary pool", %{oracle: oracle} do
    cases = cases()
    ...
    failures =
      cases
      |> Enum.with_index()
      |> Enum.flat_map(fn {[a1] = args, index} ->
        spec = oracle_call(oracle, args)
        impl = Feat.Shen.Term.to_shen(Feat.Impl.classify(a1))
