...
//
// The call predicate is spelled `calls` because `call` is a Shen
// system function: loading a fact file that used `call` into a Shen
// host failed with "call is a system function", which kept the primary
// engine from ever running. See W6 in the roadmap.
//
...
func (fs *FactSet) WriteFacts() string {
	...
	b.WriteString("     (calls caller-sym callee-sym)\n\n")
...
func ParseFacts(src string) (*FactSet, error) {
	...
	for _, form := range splitForms(src) {
		...
		// `calls`, not `call`: `call` is a Shen system function (arity
		// 5), so a fact file spelling it that way cannot be loaded
		// into a Shen host at all — `(define call …)` in the Prolog
		// stdlib was rejected with "call is a system function", which
		// is why the Shen engine had never run. `call` is still
		// accepted on *read* so a fact file an older `sb index` left
		// under .sb/ does not have to be regenerated by hand.
		case "calls", "call":
			if len(toks) != 3 {
				return nil, fmt.Errorf("flow: (%s …) wants 2 arguments, got %d", toks[0], len(toks)-1)
			}
