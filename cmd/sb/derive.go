...
`)
	...
	// Elixir specs are generated by `shengen-ex derive` and run by `mix test`
	// against the Shen define evaluated in-process by shen-erl.
	var exBin string
	var exTestFiles []string
	for _, s := range cfg.DeriveSpecs {
		if s.Lang == "elixir" && exBin == "" {
			var err error
			if exBin, err = FindShengenEx(); err != nil {
				fmt.Fprintf(os.Stderr, "sb derive: %v\n", err)
				os.Exit(1)
			}
		}
	...
	for i, spec := range cfg.DeriveSpecs {
		...
		case "elixir":
			exTestFiles = append(exTestFiles, spec.OutFile)
			runCmd = exBin
			runArgs = elixirDeriveArgs(cfg, spec)
			runDir = ""
			tempGlob = "shen-derive-*.exs"
		default:
			fmt.Fprintf(os.Stderr, "sb derive: spec[%d]: unknown lang %q (want go, ts or elixir)\n", i, spec.Lang)
			os.Exit(1)
		}

		if spec.Seed != 0 && spec.Lang != "elixir" {
	...
	// Elixir specs → one `mix test <files>` run (the shen-erl oracle is
	// shared by every generated test module in the run).
	if len(exTestFiles) > 0 {
		sort.Strings(exTestFiles)
		mixArgs := append([]string{"test"}, exTestFiles...)
		if *verbose {
			printDeriveCommand("", "mix", mixArgs)
		}
		if err := runInDir("", "mix", mixArgs...); err != nil {
			fmt.Fprintf(os.Stderr, "sb derive: mix test failed: %v\n", err)
			testFailed = true
		}
	}
...
// elixirDeriveArgs maps a [[derive.specs]] entry onto `shengen-ex derive`:
// impl_pkg is the Elixir module, impl_func the function, guard_pkg the
// guard namespace. StreamData properties are emitted when mix.exs depends
// on :stream_data (read from the committed mix.exs, so output stays
// deterministic).
func elixirDeriveArgs(cfg *Config, spec DeriveSpec) []string {
	ns := spec.GuardPkg
	if ns == "" {
		ns = cfg.Pkg
	}
	...
	if data, err := os.ReadFile("mix.exs"); err == nil && bytes.Contains(data, []byte(":stream_data")) {
		args = append(args, "--stream-data")
	}
	return args
}
