...
func generateDefineHelpers(b *strings.Builder, st *SymbolTable, bt *BrandTable) {
	...
	for {
		...
		for _, def := range ordered {
			...
			generateOneDefine(b, def, resolved, st, bt)
...
func generateOneDefine(b *strings.Builder, def *Define, resolved *DefineResolved, st *SymbolTable, bt *BrandTable) {
	...
	// Function signature. With brands on, a helper over branded types
	// is declared generic over exactly the brands its parameters need.
	brandParams := defineBrandParams(st, bt, resolved.ParamTypes)
	var params []string
	for _, p := range resolved.ParamTypes {
		...
		if bt != nil {
			...
			if p.IsList && p.ElemShenType != "" {
				shenType = "(list " + p.ElemShenType + ")"
			}
			if rendered := goTypeWithBrands(st, bt, shenType, nil); rendered != "" && bt.IsBranded(resolveAliases(st, strings.TrimSuffix(strings.TrimPrefix(shenType, "(list "), ")"))) {
				goType = rendered
			}
		}
	...
	b.WriteString(fmt.Sprintf("func %s%s(%s) bool {\n", resolved.GoName, brandDeclSuffix(brandParams), strings.Join(params, ", ")))
...
type GeneratedType struct {
	...
	// ShenType is the resolved conclusion type name — the key under
	// which the symbol table and brand table know this type. It differs
	// from Name whenever the datatype block is named after the rule
	// rather than after its conclusion (balance-invariant →
	// balance-checked).
	ShenType string
	Rule     Rule
}
...
func classify(dt Datatype, st *SymbolTable) []GeneratedType {
	...
	for _, r := range dt.Rules {
		...
		out = append(out, GeneratedType{Name: dt.Name, GoName: toPascalCase(typeName), ShenType: typeName, Category: cat, Rule: r})
	}
	return out
}

// generateGo emits the guards file with brands disabled. Kept as the
// pre-W1 entry point so `--no-brands` output is byte-identical.
func generateGo(types []Datatype, st *SymbolTable, pkg string, specPath string) string {
	return generateGoBrands(types, st, pkg, specPath, nil)
}

// generateGoBrands emits the guards file. When bt is non-nil, every
// generated type carries an unexported witness field and every proof
// type carries the phantom brand parameters bt inferred. See
// brand_emit.go for what the two mechanisms defeat.
func generateGoBrands(types []Datatype, st *SymbolTable, pkg string, specPath string, bt *BrandTable) string {
	var b strings.Builder
	...
	for concType, variants := range st.SumTypes {
		...
		b.WriteString(fmt.Sprintf("type %s%s interface {\n\t%s()\n}\n\n",
			goIface, brandDeclSuffix(bt.Params(concType)), markerMethod))
	...
	for _, dt := range types {
		for _, gt := range classify(dt, st) {
			...
			switch gt.Category {
			case "wrapper":
				generateWrapper(&b, gt, st, bt)
			case "constrained":
				generateConstrained(&b, gt, st, bt)
			case "composite":
				generateComposite(&b, gt, st, bt)
			case "guarded":
				generateGuarded(&b, gt, st, bt)
			case "alias":
				generateAlias(&b, gt, st, bt)
			}
			...
			if sumTypeVariants[gt.Name] {
				...
				for concType := range st.SumTypes {
					for _, v := range st.SumTypes[concType] {
						if v == gt.Name {
							goIface := toPascalCase(concType)
							b.WriteString(fmt.Sprintf("func (t %s%s) is%s() {}\n\n",
								gt.GoName, brandArgSuffix(bt.Params(gt.ShenType)), goIface))
						}
					}
				}
			}
...
func generateWrapper(b *strings.Builder, gt GeneratedType, st *SymbolTable, bt *BrandTable) {
	...
	emitWitnessField(b, bt)
	b.WriteString(fmt.Sprintf("\tv %s\n}\n\n", goType))
	b.WriteString(fmt.Sprintf("func New%s(x %s) %s { return %s{%sv: x} }\n\n",
		gt.GoName, goType, gt.GoName, gt.GoName, mintInline(bt)))
	guard := emitAccessorWitnessCheck(bt, gt.GoName)
	b.WriteString(fmt.Sprintf("func (t %s) Val() %s { %sreturn t.v }\n\n", gt.GoName, goType, guard))
	if goType == "string" {
		b.WriteString(fmt.Sprintf("func (t %s) String() string { %sreturn t.v }\n\n", gt.GoName, guard))
	}
}

func generateConstrained(b *strings.Builder, gt GeneratedType, st *SymbolTable, bt *BrandTable) {
	goType := shenTypeToGo(gt.Rule.Premises[0].TypeName)
	...
	b.WriteString(fmt.Sprintf("\treturn %s{%sv: x}, nil\n}\n\n", gt.GoName, mintInline(bt)))
	b.WriteString(fmt.Sprintf("func (t %s) Val() %s { %sreturn t.v }\n\n",
		gt.GoName, goType, emitAccessorWitnessCheck(bt, gt.GoName)))
}
...
func generateComposite(b *strings.Builder, gt GeneratedType, st *SymbolTable, bt *BrandTable) {
	bi := bt.Lookup(gt.ShenType)
	if bt != nil && bi == nil {
		bi = &BrandInfo{ShenName: gt.ShenType, GoName: gt.GoName}
	}
	decl := gt.GoName
	self := gt.GoName
	if bt != nil {
		decl = gt.GoName + brandDeclSuffix(bi.Params)
		self = gt.GoName + brandArgSuffix(bi.Params)
	}
	b.WriteString(fmt.Sprintf("type %s struct {\n", decl))
	emitWitnessField(b, bt)
	var params []string
	for i, p := range gt.Rule.Premises {
		goType := premiseGoType(st, bt, bi, i, p)
		b.WriteString(fmt.Sprintf("\t%s %s\n", toCamelCase(p.VarName), goType))
		params = append(params, fmt.Sprintf("%s %s", toCamelCase(p.VarName), goType))
	}
	b.WriteString("}\n\n")
	b.WriteString(fmt.Sprintf("func New%s(%s) %s {\n", ctorName(gt.GoName, bt, bi), strings.Join(params, ", "), self))
	emitWitnessChecks(b, st, bt, gt.Rule.Premises)
	b.WriteString(fmt.Sprintf("\treturn %s{\n", self))
	b.WriteString(mintField(bt))
	for _, p := range gt.Rule.Premises {
	...
	guard := emitAccessorWitnessCheck(bt, gt.GoName)
	for i, p := range gt.Rule.Premises {
		b.WriteString(fmt.Sprintf("func (t %s) %s() %s { %sreturn t.%s }\n\n",
			self, toPascalCase(p.VarName), premiseGoType(st, bt, bi, i, p), guard, toCamelCase(p.VarName)))
	}
}

// premiseGoType renders premise i of a rule at the brand arguments the
// inference unified it into (so `check BalanceChecked[B]` rather than
// `check BalanceChecked[whatever it calls its own parameter]`).
func premiseGoType(st *SymbolTable, bt *BrandTable, bi *BrandInfo, i int, p Premise) string {
	if bt == nil {
		return shenTypeToGo(p.TypeName)
	}
	var args []string
	if bi != nil && i < len(bi.PremiseArgs) {
		args = bi.PremiseArgs[i]
	}
	return goTypeWithBrands(st, bt, p.TypeName, args)
}

// ctorName renders the constructor's name plus its own type-parameter
// list when the produced type is branded.
func ctorName(goName string, bt *BrandTable, bi *BrandInfo) string {
	if bt == nil || bi == nil {
		return goName
	}
	return goName + brandDeclSuffix(bi.Params)
}

func generateGuarded(b *strings.Builder, gt GeneratedType, st *SymbolTable, bt *BrandTable) {
	bi := bt.Lookup(gt.ShenType)
	if bt != nil && bi == nil {
		bi = &BrandInfo{ShenName: gt.ShenType, GoName: gt.GoName}
	}
	...
	for _, v := range gt.Rule.Verified {
		if v.RuntimeVia != "" {
			emitRuntimeViaCheckGo(b, v, gt.Name, self, shenVarNames, argNames)
			continue
		}
		goExpr, errMsg := st.verifiedToGo(v, varMap)
		safeMsg := strings.ReplaceAll(errMsg, "%", "%%")
		safeMsg = strings.ReplaceAll(safeMsg, `"`, `\"`)
		b.WriteString(fmt.Sprintf("\tif %s {\n\t\treturn %s{}, fmt.Errorf(\"%s\")\n\t}\n", negateGoExpr(goExpr), self, safeMsg))
	}
	b.WriteString(fmt.Sprintf("\treturn %s{\n", self))
	b.WriteString(mintField(bt))
	for _, p := range gt.Rule.Premises {
...
func generateAlias(b *strings.Builder, gt GeneratedType, st *SymbolTable, bt *BrandTable) {
	if bt == nil {
		b.WriteString(fmt.Sprintf("type %s = %s\n\n", gt.GoName, shenTypeToGo(gt.Rule.Premises[0].TypeName)))
		return
	}
	// A branded alias needs a generic type alias (Go 1.24+), since the
	// target's brand has to be nameable through the alias.
	bi := bt.Lookup(gt.ShenType)
	if bt != nil && bi == nil {
		bi = &BrandInfo{ShenName: gt.ShenType, GoName: gt.GoName}
	}
	b.WriteString(fmt.Sprintf("type %s%s = %s\n\n",
		gt.GoName, brandDeclSuffix(bi.Params),
		goTypeWithBrands(st, bt, gt.Rule.Premises[0].TypeName, bi.Params)))
}
...
func generateDBWrappers(types []Datatype, st *SymbolTable, pkg string, specPath string, bt *BrandTable) string {
	var wrappers []struct {
	...
	for _, w := range wrappers {
		...
		b.WriteString(fmt.Sprintf("func New%s%s(db *sql.DB, proof %s%s) %s {\n",
			wrapperName, brandDeclSuffix(w.brands), w.proofGoName, brandArgSuffix(w.brands), wrapperName))
