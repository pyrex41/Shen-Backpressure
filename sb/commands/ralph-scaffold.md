...
**Ralph orchestrator** (e.g., `cmd/ralph/main.go` for Go, `ralph.ts` for TS, or a shell script) — runs five gates: shengen → test → build → shen-check → tcb-audit. Harness set from Step 1.
